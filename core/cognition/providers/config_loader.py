...
VALID_EMBEDDER_TYPES = {"ollama"}
VALID_POSTURES = {"conservative", "balanced", "aggressive"}

# Default model tiers for complexity-based routing (shared with router.py)
...
def _default_config() -> dict[str, Any]:
    ...
    return {
        ...
        "model_tiers": dict(DEFAULT_MODEL_TIERS),
        "costs": {
...
def load_providers_config(path: Path | None = None) -> dict[str, Any]:
    ...
    config.setdefault("embedder", defaults["embedder"])
...
def _validate(config: dict) -> None:
    ...
    # Validate embedder type if present
    embedder = config.get("embedder", {})
    if embedder:
        etype = embedder.get("type")
        if etype is not None and etype not in VALID_EMBEDDER_TYPES:
            raise ConfigError(
                f"Invalid embedder type '{etype}' — must be one of: {', '.join(sorted(VALID_EMBEDDER_TYPES))}"
            )
