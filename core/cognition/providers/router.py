...
from core.cognition.providers.config_loader import DEFAULT_MODEL_TIERS
...
_COMPLEXITY_SCORE_HIGH = 4      # score >= this -> COMPLEX
_COMPLEXITY_SCORE_MODERATE = 2  # score >= this -> MODERATE
...
class PrivacyAwareRouter:
    ...
    def __init__(
        ...
        self._model_tiers = model_tiers or DEFAULT_MODEL_TIERS
        self.last_routed_provider: str | None = None
    ...
    def route(
        ...
        **kwargs,
        ...
        tier = self._classifier.classify(messages, scope="user_input")

        target_name = self._resolve_provider(provider, cloud_name, all_content, tier)

        # Room provider allowlist enforcement
        target_name = self._enforce_allowed_providers(target_name, allowed_providers)
        ...
        self.last_routed_provider = target_name
    ...
    def route_stream(
        ...
        **kwargs,
        ...
        Sets self.last_routed_provider after resolving the actual target.
        """
        all_content = " ".join(m.content for m in messages)
        tier = self._classifier.classify(messages, scope="user_input")
    ...
    def _resolve_provider(
        ...
        T-120b: When the resolved provider is unavailable (e.g. Ollama stopped),
        automatically promotes to the first available cloud provider.
        """
        if explicit:
            return self._check_availability(explicit, cloud_name)

        cloud = cloud_name or self._find_cloud_provider()

        if self.posture == RoutingPosture.CONSERVATIVE:
            return self._check_availability(self._local_name, cloud_name)
    ...
    def _check_availability(self, target: str, cloud_name: str | None = None) -> str:
        """If *target* is unavailable, promote to cloud. Otherwise return target."""
        provider = self._registry.get(target)
        if provider is None or provider.is_available():
            return target
        # Target is unavailable — try cloud fallback
        cloud = cloud_name or self._find_cloud_provider()
        if cloud:
            log.info("Provider '%s' unavailable — promoting to cloud: %s", target, cloud)
            return cloud
        return target  # no cloud available, caller will handle the error

    def _enforce_allowed_providers(
        self, target_name: str, allowed_providers: list[str] | None,
    ) -> str:
        """Enforce Room's provider allowlist. Falls back to local if target is disallowed."""
        if allowed_providers is None:
            return target_name
        if target_name in allowed_providers:
            return target_name
        # Target disallowed — try to find an allowed alternative
        if self._local_name in allowed_providers:
            log.info("Provider '%s' not in Room allowlist %s — falling back to local", target_name, allowed_providers)
            return self._local_name
        # Local also not allowed (shouldn't happen with validation) — use first allowed
    ...
    def _classify_complexity(self, content: str) -> str:
        ...
        if score >= _COMPLEXITY_SCORE_HIGH:
            return "COMPLEX"
        if score >= _COMPLEXITY_SCORE_MODERATE:
            return "MODERATE"
        return "SIMPLE"
