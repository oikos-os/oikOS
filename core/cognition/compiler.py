...
Identity tier gets a fixed non-competitive allocation loaded first.
...
# Maximum number of search results to retrieve per tier slice
_SEARCH_LIMIT = 20
...
def fill_slice(
    ...
    """
    results = search_tier(query, tier, limit=_SEARCH_LIMIT, path_filter=path_filter, exclude_filter=exclude_filter, tag_filter=tag_filter)
...
def _extract_topic_keys(header_path: str, content: str) -> set[str]:
    ...
    Pulls from the header path (e.g. "PROJECT: PROBLEM (AUTHOR)") and
...
def compile_context(
    ...
    for name, tier in tier_map.items():
        ...
        s = fill_slice(query, tier, name, budget, exclude_chunk_ids=exclude, path_filter=path_filter, exclude_filter=exclude_filter, tag_filter=tag_filter)
        ...
        if underfilled:
            ...
            for s in underfilled:
                ...
                expanded = fill_slice(
                    ...
                    path_filter=path_filter, exclude_filter=exclude_filter,
