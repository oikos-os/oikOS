...
def fill_slice(
    ...
    """
    results = search_tier(query, tier, limit=20, path_filter=path_filter, exclude_filter=exclude_filter, tag_filter=tag_filter)
...
def _extract_topic_keys(header_path: str, content: str) -> set[str]:
    ...
    Pulls from the header path (e.g. "PROJECT: PROBLEM (USER)") and
