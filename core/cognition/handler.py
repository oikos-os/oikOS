...
def _prepare_query(
    ...
    # 3. Compile context (with Room vault scoping)
    path_filter = None
    exclude_filter = None
    tag_filter = None
    try:
        ...
        pass  # Room system not initialized — use unscoped search
        ...
        try:
            ...
            if not model_override:
                try:
                    ...
                    if room.model.provider and not pii_blocked:
                        target_provider = room.model.provider
                except (ImportError, ValueError):
