...
class RoomLimits(BaseModel):
    ...
    vault_index_limit: int | None = None
    ...
    @field_validator("vault_index_limit")
    @classmethod
    def _validate_vault_index_limit(cls, v: int | None) -> int | None:
        if v is not None:
            if v < 10:
                raise ValueError("vault_index_limit minimum is 10")
            if v > 500:
                raise ValueError("vault_index_limit maximum is 500")
        return v
...
class RoomConfig(BaseModel):
    ...
    @classmethod
    def _validate_allowed_providers(cls, v: list[str] | None) -> list[str] | None:
        if v is not None and len(v) == 0:
            raise ValueError("allowed_providers cannot be empty — a Room needs at least one provider")
        return v
