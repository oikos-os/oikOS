"""Drift Detector — three-tier escalation nudge generation."""
...
def parse_projects(projects_path: Path | None = None) -> list[dict]:
    ...
    Each project header like '## 1. PROJECT TRENDY DECAY (THE LABEL)'
...
def build_domain_keyword_map(projects_path: Path | None = None) -> dict[str, set[str]]:
    """Build domain -> keyword set mapping from PROJECTS.md.

    Returns e.g. {"trendy decay": {"trendy", "decay"}, "oikos": {"oikos"}, ...}
...
def _infer_domain_for_deadline(deadline: dict, domain_map: dict[str, set[str]]) -> str | None:
    ...
    for domain, keywords in domain_map.items():
        # Direct artist match — artist name appears in a domain's keyword set
    ...
    # (all current GOALS.md deadlines are release dates = Trendy Decay)
...
def get_session_activity(
    ...
    if domain_map is None:
        domain_map = build_domain_keyword_map()
