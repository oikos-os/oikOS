...
# Framework vault entities — product-level terms that indicate successful
# retrieval regardless of operator. Owner-specific entities (project names,
# persona aliases, collaborators) are loaded from owner_identity.toml and
# merged at first use.
_FRAMEWORK_VAULT_ENTITY_PHRASES: tuple[str, ...] = (
    "vantablack", "oikos", "kairos", "gemini", "claude",
)

_vault_entity_cache: list[str] | None = None


def _get_vault_entity_phrases() -> list[str]:
    global _vault_entity_cache
    if _vault_entity_cache is None:
        identity = load_owner_identity()
        _vault_entity_cache = list(_FRAMEWORK_VAULT_ENTITY_PHRASES) + [
            e.lower() for e in identity.vault_entities
        ]
    return _vault_entity_cache


def _reset_vault_entity_cache() -> None:
    """Clear the merged entity cache. Used by tests."""
    global _vault_entity_cache
    _vault_entity_cache = None
...
# with the original flat -10 system at median LLM response length).
...
def score_heuristic(response_text: str) -> float:
    ...
    vault_bonus = VAULT_BONUS if any(p in text_lower for p in _get_vault_entity_phrases()) else 0.0
