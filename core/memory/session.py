...
Session log layout (per session):
    logs/sessions/{room_id}/YYYY-MM-DD/
...
def _get_room_id() -> str:
    """Get active room ID, defaulting to 'home'."""
    try:
...
def _session_dir(date_str: str, room_id: str | None = None) -> Path:
    """Return SESSIONS_DIR/{room_id}/{date_str}/, creating if needed."""
    import re
    room_id = room_id or _get_room_id()
    if not re.match(r"^[a-z0-9][a-z0-9_-]{0,31}$", room_id):
        room_id = "home"
    d = SESSIONS_DIR / room_id / date_str
    d.mkdir(parents=True, exist_ok=True)
    return d


def _session_log_path(session_id: str, started_at: str, room_id: str | None = None) -> Path:
    """Return path to the JSONL interaction log for a session."""
    date_str = datetime.fromisoformat(started_at).strftime("%Y-%m-%d")
    return _session_dir(date_str, room_id=room_id) / f"SESSION-{session_id}.jsonl"


def _session_summary_path(session_id: str, started_at: str, room_id: str | None = None) -> Path:
    """Return path to the JSON summary file for a session."""
    date_str = datetime.fromisoformat(started_at).strftime("%Y-%m-%d")
    return _session_dir(date_str, room_id=room_id) / f"SESSION-{session_id}_summary.json"
...
def _append_to_log(session_id: str, started_at: str, entry: dict, room_id: str | None = None) -> None:
    """Append one JSON record to the session's JSONL file."""
    log_path = _session_log_path(session_id, started_at, room_id=room_id)
...
def _generate_summary(state: dict) -> dict:
    ...
    log_path = _session_log_path(state["session_id"], state["started_at"], room_id=state.get("room_id"))
    ...
    try:
        ...
        log.debug("session duration calc suppressed: %s", e)
...
def get_or_create_session() -> dict:
    ...
    state = {
        ...
        "timeout_minutes": get_setting("session_timeout"),
        "interaction_count": 1,
...
def _archive_session(state: dict) -> None:
    ...
    summary_path = _session_summary_path(state["session_id"], state["started_at"], room_id=state.get("room_id"))
...
def log_interaction(
    ...
    Args:
        ...
        room_id: Room that owns this session (defaults to active room).
    """
    entry = {
        "type": "query",
    ...
    _append_to_log(session_id, session_started_at, entry, room_id=room_id)
...
def _find_session_jsonl(session_id: str) -> Path | None:
    """Find the JSONL log for a session across all room/date directories."""
    if not SESSIONS_DIR.exists():
        return None
    for room_dir in SESSIONS_DIR.iterdir():
        if not room_dir.is_dir() or room_dir.name.startswith("."):
            continue
        for date_dir in room_dir.iterdir():
            if not date_dir.is_dir() or date_dir.name.startswith("."):
                continue
            candidate = date_dir / f"SESSION-{session_id}.jsonl"
            if candidate.exists():
                return candidate
    return None
...
def list_recent_sessions(limit: int = 20, room_id: str | None = None) -> list[dict]:
    ...
    Args:
        ...
        room_id: Filter to this room. If the room has session_isolation=False,
                 shows all rooms instead.
    """
    seen: set[str] = set()
    summaries: list[dict] = []
    if not SESSIONS_DIR.exists():
    ...
    # Respect session_isolation setting
    if room_id:
        try:
            from core.rooms.manager import get_room_manager
            room = get_room_manager().get_room(room_id)
            if not room.limits.session_isolation:
                room_id = None  # Show all sessions
        except (ImportError, ValueError):
            pass

    # Determine room directories to scan
    if room_id:
        room_dirs = [SESSIONS_DIR / room_id]
    else:
        room_dirs = [d for d in SESSIONS_DIR.iterdir() if d.is_dir() and not d.name.startswith(".")]

    # Collect date dirs across selected rooms
    date_dirs: list[Path] = []
    for rd in room_dirs:
        if rd.exists():
            date_dirs.extend(d for d in rd.iterdir() if d.is_dir() and not d.name.startswith("."))
...
def load_session_transcript(session_id: str) -> list[dict]:
    ...
    log_file = _find_session_jsonl(session_id)
    if not log_file:
        return []
    entries = []
    try:
