...
def _extract_frontmatter_tags(text: str) -> tuple[list[str], str]:
    """Extract tags from YAML frontmatter. Returns (tags, content_without_frontmatter)."""
...
def chunk_markdown(file_path: Path) -> list[VaultChunk]:
    ...
    tags, text = _extract_frontmatter_tags(text)
