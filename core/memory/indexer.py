...
def chunks_to_records(chunks: list[VaultChunk]) -> list[dict]:
    ...
    if not EmbedderRegistry.get_instance().is_available():
        log.warning("Embedder unavailable — skipping chunk embedding")
        return []
...
def index_vault(full_rebuild: bool = False) -> dict:
    ...
    _schema_rebuild_done = False

    def _safe_add(tbl: lancedb.table.LanceTable, records: list) -> lancedb.table.LanceTable:
        """Add records, falling back to full rebuild on schema mismatch."""
        nonlocal _schema_rebuild_done
        try:
            tbl.add(records)
            return tbl
        except ValueError as e:
            if "not found in target schema" in str(e):
                log.warning("Index schema outdated — triggering full rebuild: %s", e)
                db.drop_table(TABLE_NAME)
                new_tbl = get_or_create_table()
                all_chunks: list = []
                for f in vault_files:
                    all_chunks.extend(chunk_markdown(f))
                if all_chunks:
                    new_records = chunks_to_records(all_chunks)
                    new_tbl.add(new_records)
                    stats["added"] = len(new_records)
                stats["deleted"] = 0
                stats["skipped"] = 0
                _schema_rebuild_done = True
                return new_tbl
            raise
    ...
    if full_rebuild:
        ...
        for f in vault_files:
            ...
            if chunks:
                ...
                table = _safe_add(table, records)
                if _schema_rebuild_done:
                    break  # Full rebuild already processed all files
                stats["added"] += len(records)
