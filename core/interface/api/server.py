...
def create_app(dev: bool = False) -> FastAPI:
    ...
    from core.interface.api.routes.auth import router as auth_router
    from core.interface.api.routes.rooms import router as rooms_router
    from core.interface.api.routes.notifications import router as notifications_router
    ...
    from core.interface.api.routes.onboarding import router as onboarding_router
    app.include_router(onboarding_router, prefix="/api/onboarding")  # PUBLIC — no auth

    # Google OAuth callback — must be at /auth/google/callback (matches GCP redirect URI)
    from fastapi import APIRouter as _Router
    google_callback_router = _Router()

    @google_callback_router.get("/auth/google/callback")
    async def google_oauth_callback(code: str = "", state: str = "", error: str = ""):
        ...
        if error:
            return HTMLResponse(f"<h3>Google OAuth Error: {escape(error)}</h3><p><a href='/'>Return to oikOS</a></p>", status_code=400)

        if not code:
            return HTMLResponse("<h3>Missing authorization code</h3><p><a href='/'>Return to oikOS</a></p>", status_code=400)

        from core.auth.google_oauth import exchange_code, verify_state

        if not verify_state(state):
            return HTMLResponse("<h3>Invalid state — possible CSRF attack</h3>", status_code=403)

        tokens = exchange_code(code)
        if not tokens:
            return HTMLResponse("<h3>Failed to exchange authorization code</h3><p><a href='/'>Return to oikOS</a></p>", status_code=500)

        # Redirect back to oikOS
        return RedirectResponse(url="/", status_code=302)

    app.include_router(google_callback_router)  # PUBLIC — OAuth callback
    ...
    app.include_router(rooms_router, prefix="/api/rooms", dependencies=auth_dep)
    app.include_router(auth_router, prefix="/api/auth", dependencies=auth_dep)
    ...
    if not dev:
        ...
        if dist.exists():
            ...
            async def serve_spa(full_path: str):  # noqa: ARG001
                ...
                # CRIT-001: prevent path traversal out of dist/
...
# Alias used by tests and tooling that expect build_app()
