...
def _room_has_model_override() -> bool:
    try:
        from core.rooms.manager import get_room_manager
        room = get_room_manager().get_active_room()
        return bool(room.model.model or room.model.provider)
    except Exception:
        return False
        ...
        for chunk in execute_query_stream(
            ...
            if chunk["done"]:
                ...
                payload = {
                    ...
                    "auto": req.model is None and not _room_has_model_override(),
                    "confidence": resp.confidence if resp else None,
                    "pii_scrubbed": resp.pii_scrubbed if resp else False,
                    "routing_trace": resp.routing_trace if resp else None,
                    "pipeline": {
...
_ALLOWED_CATEGORIES = {"general", "code", "research", "creative", "analysis"}
...
def chat_suggestions(category: str = Query(..., min_length=1)):
    ...
    try:
        ...
        log.debug("suggestion system state suppressed: %s", e)
    try:
        ...
        log.debug("suggestion vault search suppressed: %s", e)

    context = "\n".join(context_parts) if context_parts else "No additional context available."

    prompt = (
        f"You are KAIROS PRIME, a sovereign AI system. The user clicked the \"{category}\" quick-action button.\n"
...
def chat_history(limit: int = Query(20, ge=1, le=100)):
    ...
    try:
        ...
        room_id = None
    return list_recent_sessions(limit=limit, room_id=room_id)
