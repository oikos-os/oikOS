...
def _boot(prod: bool = False) -> None:
    ...
    def _cleanup():
        for p in children:
            try:
                if p.poll() is None:
                    # Kill entire process tree on Windows (shell=True spawns cmd.exe wrappers)
...
def serve(port: int, dev: bool, room_id: str | None, no_boot: bool):
...
def provider_list():
    ...
    for name in reg.list_all():
        ...
        console.print(f"  {display} ({name}): {status}{default}")
...
def room_create(room_id: str, name: str, description: str | None, template: str | None):
    ...
    try:
        ...
        console.print(f"  Room [bold]{room_id}[/] created.")
    except ValueError as e:
        console.print(f"[red]{e}[/]")
        raise SystemExit(1)
...
def room_edit(room_id: str, name: str | None, description: str | None, add_toolset: tuple, remove_toolset: tuple):
