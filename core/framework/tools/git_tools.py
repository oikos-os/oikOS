...
@oikos_tool(
    ...
    read_only=True,
    search_hint="git status branch changes staged modified",
...
def git_status(repo_path: str) -> dict:
    try:
        ...
        log.warning("[git_status] scope validation failed: %s", exc)
...
@oikos_tool(
    ...
    concurrent_safe=True,
    read_only=True,
    search_hint="git log history commits recent changes",
    group="version_control",
)
def git_log(repo_path: str, count: int = 10) -> list[dict]:
    try:
        ...
        log.warning("[git_log] scope validation failed: %s", exc)
        return [{"status": "error", "message": "Operation failed."}]
