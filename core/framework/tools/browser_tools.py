...
@oikos_tool(
    ...
    read_only=True,
    search_hint="fetch web page url download content http",
    group="web",
)
async def web_fetch(url: str, max_tokens: int = 6000) -> dict:
    try:
        ...
        log.warning("[web_fetch] URL validation failed: %s", exc)
...
@oikos_tool(
    ...
    concurrent_safe=True,
    read_only=True,
    search_hint="extract scrape elements css xpath selector",
    group="web",
)
async def web_extract(url: str, selector: str, selector_type: str = "css") -> dict:
    try:
        ...
        log.warning("[web_extract] URL validation failed: %s", exc)
        return {"status": "error", "message": "Operation failed."}
    mgr = _get_manager()
    try:
        await mgr.rate_limiter.acquire(url)
        ...
        if "playwright install" in str(exc).lower():
            return {"status": "error", "message": "Playwright not installed — run: playwright install"}
...
@oikos_tool(
    ...
    concurrent_safe=True,
    read_only=True,
    search_hint="screenshot capture page image headless browser",
    group="web",
)
async def web_screenshot(url: str, full_page: bool = False, width: int = 1280, height: int = 720) -> dict:
    try:
        ...
        log.warning("[web_screenshot] URL validation failed: %s", exc)
        return {"status": "error", "message": "Operation failed."}
    mgr = _get_manager()
    try:
        await mgr.rate_limiter.acquire(url)
...
@oikos_tool(
    ...
    toolset="browser",
    search_hint="navigate browse click fill interact automate page",
    group="web",
)
async def web_navigate(url: str, actions: list[dict] | str = "") -> dict:
    try:
        ...
        log.warning("[web_navigate] URL validation failed: %s", exc)
        return {"status": "error", "message": "Operation failed."}
    mgr = _get_manager()
...
@oikos_tool(
    ...
    autonomy=AutonomyLevel.ASK_FIRST,
    toolset="browser",
    search_hint="monitor watch page changes detect diff alert",
    group="web",
)
async def web_monitor(url: str, selector: str = "", interval_minutes: int = 30) -> dict:
    try:
        ...
        log.warning("[web_monitor] URL validation failed: %s", exc)
        return {"status": "error", "message": "Operation failed."}
    mgr = _get_manager()
    try:
        await mgr.rate_limiter.acquire(url)
