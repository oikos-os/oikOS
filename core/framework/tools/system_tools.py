...
@oikos_tool(
    ...
    read_only=True,
    search_hint="system health status ollama daemon providers",
    group="system",
)
...
@oikos_tool(
    ...
    concurrent_safe=True,
    read_only=True,
    search_hint="fsm state idle active asleep current",
    group="system",
)
def state_get() -> dict:
...
@oikos_tool(
    ...
    toolset="system",
    search_hint="fsm state transition change idle active asleep",
    group="system",
)
...
@oikos_tool(
    ...
    autonomy=AutonomyLevel.ASK_FIRST,
    toolset="system",
    search_hint="security gauntlet adversarial probes test audit",
    group="system",
)
...
@oikos_tool(
    ...
    autonomy=AutonomyLevel.ASK_FIRST,
    toolset="system",
    search_hint="generate adversarial probes security novel attack",
    group="system",
)
...
@oikos_tool(
    ...
    autonomy=AutonomyLevel.SAFE,
    toolset="system",
    search_hint="session start begin resume initialize",
    group="system",
)
...
@oikos_tool(
    ...
    autonomy=AutonomyLevel.ASK_FIRST,
    toolset="system",
    search_hint="session close end stop finalize",
    group="system",
)
...
@oikos_tool(
    ...
    autonomy=AutonomyLevel.ASK_FIRST,
    toolset="system",
    search_hint="daemon start background service heartbeat",
    group="system",
)
...
@oikos_tool(
    ...
    autonomy=AutonomyLevel.ASK_FIRST,
    toolset="system",
    search_hint="daemon stop shutdown background service",
    group="system",
)
def daemon_stop() -> dict:
    try:
        ...
        return {"status": "error", "message": "Daemon failed to stop"}
...
@oikos_tool(
    ...
    concurrent_safe=True,
    read_only=True,
    search_hint="config settings read value configuration",
    group="system",
)
def config_get(key: str, source: str = "settings") -> dict:
    ...
    if source == "settings":
        ...
        try:
            ...
            return {"status": "error", "message": "Invalid JSON in settings.json"}
        value = data.get(key)
        if value is None:
        ...
        try:
            ...
            return {"status": "error", "message": "TOML parse error"}
        value = data.get(key)
        if value is None:
...
@oikos_tool(
    ...
    autonomy=AutonomyLevel.ASK_FIRST,
    toolset="system",
    search_hint="config settings write update set configuration",
    group="system",
)
...
@oikos_tool(
    name="oikos_notify",
    description="Send a toast notification to ARCHITECT's desktop",
    privacy=PrivacyTier.SAFE,
    autonomy=AutonomyLevel.SAFE,
    toolset="system",
    search_hint="notify alert toast message notification desktop",
    group="system",
)
