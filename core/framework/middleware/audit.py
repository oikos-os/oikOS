...
class AuditMiddleware:
    ...
    async def __call__(self, ctx: MiddlewareContext, call_next: Callable) -> Any:
        ...
        try:
            ...
            log.error("Audit middleware caught: %s: %s", type(exc).__name__, exc)
    ...
    def _write_record(self, ctx: MiddlewareContext, result: Any, error: str | None, latency_ms: int) -> None:
        ...
        autonomy_raw = ctx.tool_meta.autonomy
        if hasattr(autonomy_raw, "value"):
            autonomy_str = autonomy_raw.value
        else:
            autonomy_str = str(autonomy_raw)

        privacy_raw = ctx.extras.get("privacy_tier", ctx.tool_meta.privacy)
        if hasattr(privacy_raw, "value"):
            privacy_str = privacy_raw.value
        else:
            privacy_str = "unknown"
