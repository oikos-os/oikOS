...
class CoherenceResult(NamedTuple):
    ...
    is_coherent: bool
    identity_score: float  # 0.0-1.0, presence of KAIROS identity markers
    foreign_persona_detected: bool
    foreign_markers: list[str]
    warning_message: str | None


# KAIROS identity markers — expect these in responses
_IDENTITY_MARKERS = {
    # System names
    "kairos", "oikos", "omega", "lieutenant", "engineer", "sovereign",
    ...
    # Project names
    "example project", "example novel", "apricot", "aegis",

    # Key figures
    "user", "example_artist_2", "boy", "example_artist", "jocasta", "oedipus",

    # Domain indicators
    "acme corp", "music production", "literature", "phase",
}
...
_FOREIGN_PERSONA_PATTERNS = [
    ...
    (r"\bmy name is (?!kairos|oikos|the lieutenant)\b", "name_claim"),
...
def check_coherence(response: str, query: str = "") -> CoherenceResult:
    """
    Check if response maintains KAIROS identity coherence.
    ...
    1. Absence: Response lacks KAIROS identity markers (total departure)
    ...
    if foreign_detected and identity_density < 0.5:
        ...
        warning = (
            ...
            f"and lacks KAIROS identity (density={identity_density:.2f} markers/100 words, "
        ...
        warning = (
            ...
            f"Response has low KAIROS identity density ({identity_density:.2f} markers/100 words, "
