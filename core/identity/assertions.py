...
# ---------------------------------------------------------------------------
# Classifier prompt (R-02 v2 — 95% accuracy on local model)
# ---------------------------------------------------------------------------
...
def _classify_assertion(query: str) -> dict | None:
    """
    Call local model to classify assertion. Returns raw dict or None on failure.
...
def check_assertion(query: str) -> AssertionResult:
    ...
    2. LLM classifier (local model, p50=0.47s warm)
