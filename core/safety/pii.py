...
# Framework-level whitelist: names that are part of the oikOS product
# itself or common false-positives that aren't operator-identifying.
# Owner-specific persona names come from owner_identity.toml and are
# merged in at first use.
_FRAMEWORK_PII_WHITELIST: frozenset[str] = frozenset({
    # oikOS product terms
    "oikos", "oikos omega",

    # Financial-entity false positives
    "apple card", "discover card", "discover",
})


def _build_pii_whitelist() -> frozenset[str]:
    """Framework whitelist merged with owner personas from config."""
    identity = load_owner_identity()
    return _FRAMEWORK_PII_WHITELIST | identity.persona_whitelist()


_pii_whitelist_cache: frozenset[str] | None = None


def _get_pii_whitelist() -> frozenset[str]:
    global _pii_whitelist_cache
    if _pii_whitelist_cache is None:
        _pii_whitelist_cache = _build_pii_whitelist()
    return _pii_whitelist_cache


def _reset_whitelist_cache() -> None:
    """Clear the merged whitelist cache. Used by tests."""
    global _pii_whitelist_cache
    _pii_whitelist_cache = None
...
def _analyze_pii(text: str) -> tuple[str, list, list[PIIEntity]]:
    """Run PII analysis: normalize text, invoke Presidio, filter whitelist, build entities.

    Returns:
        (normalized_text, raw_analyzer_results, entities)
    """
    ...
    results = analyzer.analyze(
        ...
        language="en",
        score_threshold=get_setting("pii_confidence_threshold"),
    )

    # Filter whitelist (strip possessives before checking)
    whitelist = _get_pii_whitelist()
    results = [
        r for r in results
        if normalized_text[r.start:r.end].lower().rstrip("'s") not in whitelist
    ]
    ...
    return normalized_text, results, entities
...
def detect_pii(text: str) -> PIIResult:
    ...
    """
    _normalized_text, _raw_results, entities = _analyze_pii(text)
...
def scrub_pii(text: str) -> PIIResult:
    ...
    effective_text, analysis_results, entities = _analyze_pii(text)
