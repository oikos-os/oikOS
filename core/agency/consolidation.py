...
Uses consolidate_memory Fabric pattern.
...
from core.utils.text import strip_markdown_fences
...
def run_consolidation(
    ...
    try:
        ...
        log.debug("consolidation_start emit_event suppressed: %s", e)
    ...
    for i, (sfile, rel_path) in enumerate(candidates):
        ...
        result = generate_routed(prompt, model=CONSOLIDATION_MODEL)

        try:
            text = strip_markdown_fences(result.get("response", ""))
    ...
    try:
        ...
        log.debug("consolidation_complete emit_event suppressed: %s", e)
