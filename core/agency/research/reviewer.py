...
from core.interface.config import PROJECT_ROOT, TIER_PATHS
from core.memory.indexer import index_vault

log = logging.getLogger(__name__)

_STAGING_DIR = PROJECT_ROOT / "staging" / "research"
...
class ResearchReviewer:
    ...
    def approve(self, filename: str, vault_tier: str = "semantic", domain: str = "RESEARCH") -> dict:
        try:
            ...
            log.warning("[reviewer.approve] validation failed: %s", exc)
    ...
    def reject(self, filename: str) -> dict:
        try:
            ...
            log.warning("[reviewer.reject] validation failed: %s", exc)
            return {"status": "error", "message": "Operation failed."}
