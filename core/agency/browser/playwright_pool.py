"""On-demand headless Firefox lifecycle manager."""
...
class PlaywrightPool:
    ...
    async def _launch(self) -> None:
        try:
            ...
            self._browser = await self._pw_instance.firefox.launch(**launch_kwargs)
        except Exception as exc:
            self._browser = None
            if "Executable doesn't exist" in str(exc) or "executable" in str(exc).lower():
                raise RuntimeError(
                    "Playwright Firefox not installed. Run: playwright install firefox"
                ) from exc
            raise
