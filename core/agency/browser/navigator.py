    ...
    async def _execute_action(self, page, action: dict) -> dict:
        ...
        try:
            if action_type == "click":
                ...
                staging = PROJECT_ROOT / "staging" / "screenshots"
                staging.mkdir(parents=True, exist_ok=True)
                name = Path(action.get("name", "nav_screenshot")).name  # strip path traversal
            ...
            log.warning("[navigator] action %s failed: %s", action_type, exc)
            return {"type": action_type, "status": "error", "detail": "Operation failed."}
