...
generate_routed = None  # lazy-loaded
_PLACEHOLDER_RE = re.compile(r"#E\d+")
_FENCE_RE = re.compile(r"```(?:json)?\s*\n?(.*?)\n?\s*```", re.DOTALL)


def _ensure_generate_routed():
    global generate_routed
    if generate_routed is None:
        from core.cognition.inference import generate_routed as _gr
        generate_routed = _gr
...
class ReWOOPlanner:
    def plan(self, task: str, available_tools: list[str]) -> list[PlanStep]:
        ...
        try:
            resp = generate_routed(prompt, model=COMPRESSOR_MODEL)
