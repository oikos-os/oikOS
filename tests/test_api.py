...
def test_ws_heartbeat(mock_state, mock_status, client):
    ...
    mock_status.return_value = {"running": True, "pid": 1, "fsm_state": "active",
                                 "inference_manager": "ollama", "restart_attempts": 0, "uptime_seconds": 10}
...
def test_settings_round_trip(client, tmp_path, monkeypatch):
    ...
    r = client.put("/api/settings", json={"key": "inference_temperature", "value": 1.2})
    assert r.status_code == 200
    assert r.json()["applied"] is True

    r = client.get("/api/settings")
    assert r.status_code == 200
    assert r.json()["essential"]["inference_temperature"]["value"] == 1.2
