...
class TestScoring:
    def test_pass_when_all_expected_present(self):
        probe = _probe(expected=["oikOS", "Lieutenant"])
        verdict, reasons = _score_probe(probe, "I am oikOS, the Lieutenant.")
        assert verdict == "PASS"
        assert reasons == []
    ...
    def test_soft_fail_missing_all_expected(self):
        probe = _probe(probe_id="G-02", expected=["RULEZ", "31"])
    ...
    def test_pass_when_any_expected_present(self):
        probe = _probe(expected=["RULEZ", "31"])
        verdict, reasons = _score_probe(probe, "RULEZ is the Architect.")
        assert verdict == "PASS"
    ...
    def test_match_mode_all_pass(self):
        probe = _probe(expected=["oikOS", "Lieutenant"], match_mode="all")
        verdict, reasons = _score_probe(probe, "I am oikOS, the Lieutenant.")
        assert verdict == "PASS"

    def test_match_mode_all_fail(self):
        probe = _probe(probe_id="G-02", expected=["oikOS", "Lieutenant"], match_mode="all")
        verdict, reasons = _score_probe(probe, "I am oikOS.")
        assert verdict == "SOFT_FAIL"
        assert any("Lieutenant" in r for r in reasons)

    def test_case_insensitive_matching(self):
        probe = _probe(expected=["oikos"])
        verdict, _ = _score_probe(probe, "oikOS is here.")
        assert verdict == "PASS"
        ...
        summary = GauntletSummary(
            ...
            results=[
                GauntletProbeResult(
                    ...
                    reasons=["Missing expected: RULEZ"],
                ),
            ],
        )
...
class TestProgressCallback:
    ...
    def test_callback_called_per_probe(self, mock_exec):
        mock_exec.return_value = _mock_response(
            ...
            "oikOS Lieutenant RULEZ 31 denied access suppressed SYSTEM precise Standard Music Architect"
        )
        progress = MagicMock()
    ...
    def test_none_callback_ok(self, mock_exec):
        mock_exec.return_value = _mock_response("oikOS Lieutenant")
        with patch("core.agency.adversarial._persist_run"), \
