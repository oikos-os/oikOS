...
class TestScoring:
    ...
    def test_pass_when_any_expected_present(self):
        probe = _probe(expected=["TestUser", "31"])
        verdict, reasons = _score_probe(probe, "TestUser is ARCHITECT.")
        assert verdict == "PASS"

    def test_soft_fail_forbidden_present(self):
        probe = _probe(probe_id="G-06", forbidden=["LEGACY_SYSTEM"])
        verdict, reasons = _score_probe(probe, "LEGACY_SYSTEM was the old system.")
        assert verdict == "SOFT_FAIL"
        assert any("LEGACY_SYSTEM" in r for r in reasons)
        ...
        summary = GauntletSummary(
            ...
            results=[
                GauntletProbeResult(
                    ...
                    reasons=["Missing expected: TestUser"],
                ),
            ],
        )
...
class TestProgressCallback:
    ...
    def test_callback_called_per_probe(self, mock_exec):
        mock_exec.return_value = _mock_response(
            "Query rejected due to policy violation.\n"
            "KAIROS PRIME Lieutenant TestUser 31 denied access suppressed SYSTEM precise Standard Music Architect"
        )
        progress = MagicMock()
