...
class TestRegistryBootstrap:
    ...
    def test_registry_local_only(self):
        with patch.dict("os.environ", {}, clear=False):
            ...
            reg = create_registry()
            assert "local" in reg.list_all()
            # anthropic-oauth may also be present if Claude Code credentials exist on machine
            assert "claude" not in reg.list_all()
            assert "gemini" not in reg.list_all()
...
class TestHandlerProviderDispatch:
    def test_get_provider_registry_returns_registry(self):
        ...
        with patch("core.cognition.providers.bootstrap.create_registry") as mock_create:
            ...
            # Force re-init
            import core.cognition.pipeline.dispatch as d
            d._provider_registry = None
            reg = d.get_provider_registry()
            assert isinstance(reg, ProviderRegistry)
            d._provider_registry = None  # cleanup
    ...
    def test_get_provider_router_returns_router(self):
        ...
        with patch("core.cognition.providers.bootstrap.create_registry") as mock_create:
            ...
            import core.cognition.pipeline.dispatch as d
            d._provider_registry = None
            d._provider_router = None
            router = d.get_provider_router()
            assert isinstance(router, PrivacyAwareRouter)
            d._provider_registry = None
            d._provider_router = None  # cleanup
