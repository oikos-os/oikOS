    ...
    return CompiledContext(
        ...
        slices=[
            ContextSlice(
                ...
                fragments=["You are KAIROS PRIME. The Architect is TestUser, 31yo polymath."],
...
def test_e2e_01_clean_music_query(mock_render, mock_compile):
    """Clean input → local inference → KAIROS-coherent response → no blocks."""
    ...
    with patch("core.cognition.inference.get_inference_client", return_value=client):
        resp = execute_query("What genre is the industrial reggaeton sound?", force_local=True)
...
def test_e2e_05_assertion_contradiction(mock_render, mock_compile):
    ...
    identity_conflict = ContradictionResult(
        ...
        confidence=85.0, explanation="Location mismatch: vault says McNair/Herndon VA",
    )

    vault_chunks = [SearchResult(
        chunk_id="c1", source_path="vault/identity/TELOS_01.md", tier=MemoryTier.CORE,
        header_path="identity", content="Location: McNair/Herndon, VA.", relevance_score=0.9,
...
def test_e2e_06_cloud_routing(mock_router, mock_charge, mock_ceiling, mock_render, mock_compile):
    """force_cloud=True → legacy cloud dispatch → credits charged → cloud text in response.

    Note: get_provider_router is mocked to raise so the legacy cloud path is tested.
    Previously this test relied on a NameError in _execute_query_inner (model_override
    was a free variable) to fall through to the legacy path. Now that the handler
    properly passes model_override/cloud_name, the provider router must be explicitly
    bypassed to test the legacy cloud codepath.
    """
    from core.cognition.handler import execute_query
...
def test_e2e_09_identity_query_coherence(mock_render, mock_compile):
    """'Who are you?' → KAIROS-dense response → coherence passes with identity markers."""
    from core.cognition.handler import execute_query

    client = _ollama_client(
        "I am KAIROS PRIME, the Sovereign Kernel of the OIKOS. "
        "The Architect is TestUser. The Void is watching. Standing by."
    )
    with patch("core.cognition.inference.get_inference_client", return_value=client):
