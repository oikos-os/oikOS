...
class TestAnonymize:
    def test_anonymize_returns_map(self, classifier):
        pii_result = PIIResult(
            has_pii=True,
            entities=[PIIEntity(entity_type="PERSON", text="Alice", start=5, end=10, score=0.9)],
...
class TestCredentialPatterns:
    ...
    def test_aws_key_still_works(self, classifier):
        tier = classifier.classify("AWS_KEY=AKIA" + "IOSFODNN7EXAMPLE")
        assert tier == DataTier.NEVER_LEAVE
...
class TestScopeUserInput:
    """scope='user_input' classifies only role='user' messages."""

    def test_system_prompt_with_identity_user_query_safe(self, classifier):
        """System prompt has TELOS but user query is safe → SAFE."""
        messages = [
            {"role": "system", "content": "You are KAIROS. Source: vault/identity/TELOS_01.md. MISSION.md loaded."},
            {"role": "user", "content": "What is the capital of France?"},
        ]
        pii_result = PIIResult(has_pii=False, entities=[])
        with patch("core.safety.pii.detect_pii", return_value=pii_result):
            tier = classifier.classify(messages, scope="user_input")
        assert tier == DataTier.SAFE

    def test_user_query_with_never_leave_pattern(self, classifier):
        """User asks about TELOS → NEVER_LEAVE even with scope='user_input'."""
        messages = [
            {"role": "system", "content": "You are KAIROS."},
            {"role": "user", "content": "What does my TELOS say about purpose?"},
        ]
        tier = classifier.classify(messages, scope="user_input")
        assert tier == DataTier.NEVER_LEAVE

    def test_user_query_with_api_key(self, classifier):
        """User pastes an API key → NEVER_LEAVE."""
        messages = [
            {"role": "system", "content": "General assistant."},
            {"role": "user", "content": "Store this: sk-" + "1234567890abcdef1234"},
        ]
        tier = classifier.classify(messages, scope="user_input")
        assert tier == DataTier.NEVER_LEAVE

    def test_full_scope_catches_system_prompt(self, classifier):
        """scope='full' (default) catches identity in system prompt."""
        messages = [
            {"role": "system", "content": "Source: vault/identity/TELOS_01.md"},
            {"role": "user", "content": "What is 2+2?"},
        ]
        tier = classifier.classify(messages, scope="full")
        assert tier == DataTier.NEVER_LEAVE

    def test_default_scope_is_full(self, classifier):
        """No scope param → full scope (backward compat)."""
        messages = [
            {"role": "system", "content": "TELOS loaded."},
            {"role": "user", "content": "Hello"},
        ]
        tier = classifier.classify(messages)
        assert tier == DataTier.NEVER_LEAVE

    def test_empty_user_messages_fail_safe(self, classifier):
        """No role='user' messages → NEVER_LEAVE (fail-safe)."""
        messages = [
    ...
    def test_multiple_user_messages_concatenated(self, classifier):
        """Multiple user messages are concatenated for classification."""
        messages = [
            {"role": "system", "content": "Identity context with TELOS and MISSION.md"},
    ...
    def test_string_input_with_user_input_scope(self, classifier):
        """String input with scope='user_input' classifies the string directly."""
        pii_result = PIIResult(has_pii=False, entities=[])
        with patch("core.safety.pii.detect_pii", return_value=pii_result):
            tier = classifier.classify("What is the weather?", scope="user_input")
        assert tier == DataTier.SAFE

    def test_provider_message_objects(self, classifier):
        """Works with ProviderMessage objects (have .role and .content attrs)."""
        from core.interface.models import ProviderMessage
        messages = [
            ProviderMessage(role="system", content="vault/identity/TELOS loaded"),
    ...
    def test_sensitive_pii_in_user_input(self, classifier):
        """PII in user message → SENSITIVE even with scope='user_input'."""
        messages = [
            {"role": "system", "content": "TELOS identity context"},
            {"role": "user", "content": "Send an email to John Smith at john@example.com"},
        ]
        pii_result = PIIResult(
            has_pii=True,
            entities=[PIIEntity(entity_type="EMAIL_ADDRESS", text="john@example.com", start=30, end=46, score=0.9)],
        )
        with patch("core.safety.pii.detect_pii", return_value=pii_result):
            tier = classifier.classify(messages, scope="user_input")
        assert tier == DataTier.SENSITIVE
