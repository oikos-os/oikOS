...
@patch("core.cognition.pipeline.postprocess.score_response", return_value=_mock_confidence())
@patch("core.cognition.pipeline.dispatch.generate_local", return_value=_mock_generate())
@patch("core.cognition.pipeline.context.load_system_prompt", return_value="system")
@patch("core.cognition.pipeline.context.compile_context")
@patch("core.cognition.pipeline.context.render_context", return_value="context block")
@patch("core.cognition.pipeline.classify.detect_pii", return_value=_mock_pii_clean())
...
@patch("core.cognition.pipeline.context.compile_context")
@patch("core.cognition.pipeline.context.render_context", return_value="ctx")
@patch("core.cognition.pipeline.classify.scrub_pii", return_value=_mock_scrub())
@patch("core.cognition.pipeline.classify.log_detection")
@patch("core.cognition.pipeline.classify.detect_pii", return_value=_mock_pii_dirty())
...
@patch("core.cognition.pipeline.dispatch.log_routing_decision")
@patch("core.cognition.routing.route_query")
@patch("core.cognition.pipeline.postprocess.score_response", return_value=_mock_confidence(score=45.0))
@patch("core.cognition.pipeline.dispatch.generate_local", return_value=_mock_generate())
@patch("core.cognition.pipeline.context.load_system_prompt", return_value="")
@patch("core.cognition.pipeline.context.compile_context", return_value=_mock_compiled())
@patch("core.cognition.pipeline.context.render_context", return_value="ctx")
@patch("core.cognition.pipeline.classify.detect_pii", return_value=_mock_pii_clean())
@patch("core.cognition.pipeline.dispatch.check_hard_ceiling", return_value=False)
@patch("core.cognition.pipeline.dispatch.charge")
...
@patch("core.cognition.pipeline.context.render_context", return_value="ctx")
@patch("core.cognition.pipeline.classify.detect_pii", return_value=_mock_pii_clean())
@patch("core.cognition.pipeline.dispatch.check_hard_ceiling", return_value=True)
...
@patch("core.cognition.pipeline.context.compile_context", return_value=_mock_compiled())
@patch("core.cognition.pipeline.context.render_context", return_value="ctx")
@patch("core.cognition.pipeline.classify.scrub_pii", return_value=PIIResult(has_pii=True, entities=[PIIEntity(entity_type="PERSON", text="John", start=0, end=4, score=0.9)], scrubbed_text=None))
@patch("core.cognition.pipeline.classify.log_detection")
@patch("core.cognition.pipeline.classify.detect_pii", return_value=_mock_pii_dirty())
@patch("core.cognition.pipeline.dispatch.check_hard_ceiling", return_value=False)
...
def test_filter_cloud_context_strips_core_episodic():
    from core.cognition.pipeline.dispatch import filter_cloud_context

    compiled = _mock_compiled()
    with patch("core.cognition.pipeline.dispatch.render_context") as mock_render:
        mock_render.return_value = "filtered"
        result = filter_cloud_context(compiled)
...
def test_handler_nli_veto(
    ...
    with (
        ...
        resp = execute_query("Who is ARCHITECT?")
...
@patch("core.cognition.pipeline.dispatch.log_routing_decision")
@patch("core.cognition.routing.route_query")
@patch("core.cognition.pipeline.postprocess.score_response", return_value=_mock_confidence(score=70.0))
@patch("core.cognition.pipeline.dispatch.generate_local", return_value=_mock_generate())
@patch("core.cognition.pipeline.context.load_system_prompt", return_value="")
@patch("core.cognition.pipeline.context.compile_context", return_value=_mock_compiled())
@patch("core.cognition.pipeline.context.render_context", return_value="ctx")
...
def test_stream_nli_identity_veto_appends_warning(
    ...
    with (
        patch("core.cognition.pipeline.dispatch.generate_local_stream", return_value=_mock_local_stream_chunks()),
        patch("core.memory.search.hybrid_search", return_value=[]),
        patch("core.identity.contradiction.check_contradiction", return_value=_mock_identity_contradiction()),
    ):
        chunks = list(execute_query_stream("Who is ARCHITECT?"))
...
def test_prepare_query_returns_prepared_context(
    ...
    prep, trace = result
    assert isinstance(prep, PreparedContext)
    assert prep.effective_query == "What is Python?"
    assert prep.pii_scrubbed is False
    assert prep.decision.route == RouteType.LOCAL
    assert prep.system_prompt == "system"
    assert prep.qhash is not None
    assert prep.session == {"session_id": "s1", "started_at": "t0"}
    from core.cognition.pipeline.trace import RoutingTrace
    assert isinstance(trace, RoutingTrace)


@patch("core.cognition.pipeline.postprocess.score_response", return_value=_mock_confidence(score=80.0))
def test_post_process_applies_checks_in_order(mock_score):
    from core.cognition.pipeline.postprocess import post_process as _post_process
...
def test_strip_json_preamble_removes_classifier_json():
    from core.cognition.pipeline import strip_json_preamble as _strip_json_preamble

    text = '{"contains_assertion": false, "assertion_type": "none", "extracted_claim": null} The Architect is TestUser.'
    assert _strip_json_preamble(text) == "The Architect is TestUser."
