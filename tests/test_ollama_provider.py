...
class TestOllamaProviderGenerate:
    ...
    def test_generate_with_system_message(self, provider):
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.json.return_value = _mock_chat_response("I am KAIROS")
        mock_resp.raise_for_status = MagicMock()

        with patch.object(provider._client, "post", return_value=mock_resp) as mock_post:
            msgs = [
                ProviderMessage(role="system", content="You are KAIROS PRIME"),
            ...
            assert call_body["messages"][0]["content"] == "You are KAIROS PRIME"
