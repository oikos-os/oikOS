...
@patch("core.memory.embedder_registry.EmbedderRegistry.is_available", return_value=True)
@patch("core.memory.indexer.embed_batch", side_effect=_fake_embed_batch)
@patch("core.memory.indexer.LANCEDB_DIR", Path(tempfile.mkdtemp()) / "lancedb")
def test_full_rebuild(mock_embed, mock_avail):
...
@patch("core.memory.indexer.embed_batch", side_effect=_fake_embed_batch)
@patch("core.memory.indexer.LANCEDB_DIR", Path(tempfile.mkdtemp()) / "lancedb")
def test_incremental_skips_unchanged(mock_embed, mock_avail):
...
@patch("core.memory.indexer.embed_batch", side_effect=_fake_embed_batch)
@patch("core.memory.indexer.LANCEDB_DIR", Path(tempfile.mkdtemp()) / "lancedb")
def test_table_stats(mock_embed, mock_avail):
