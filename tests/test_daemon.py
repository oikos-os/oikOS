"""Tests for core.autonomic.daemon — heartbeat loop, local inference management, service lifecycle."""
...
def _reset_daemon_state():
    ...
    daemon._inference_manager = None
    daemon._restart_attempts = 0
    daemon._restart_window_start = None
...
# ── Test 3: install_service calls schtasks ────────────────────────────
...
# ── Test 5: heartbeat_tick batches all checks ────────────────────────
...
@patch("core.autonomic.daemon._check_local_inference")
@patch("core.autonomic.daemon._check_input_activity")
def test_heartbeat_tick_batches_all_checks(mock_input, mock_inference,
...
def test_prewarm_triggers_in_window(tmp_path):
    ...
    with patch("core.autonomic.daemon.DAEMON_PREWARM_DATA_FILE", data_file), \
         patch("core.autonomic.daemon._inference_manager", mock_manager), \
         patch("core.autonomic.events.emit_event"):
