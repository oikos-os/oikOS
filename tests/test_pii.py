...
def test_whitelist_filters_detect_pii(mock_analyzer_global):
    """Whitelisted terms (e.g. 'TestUser', 'Trendy Decay') are not flagged as PII."""
    text = "Ask TestUser about Trendy Decay"
    mock_analyzer = MagicMock()
    mock_analyzer.analyze.return_value = [
        _mock_analyzer_result("PERSON", 4, 9, 0.85),       # "TestUser"
        _mock_analyzer_result("ORGANIZATION", 16, 28, 0.80),  # "Trendy Decay"
    ]
    with patch("core.safety.pii.get_analyzer", return_value=mock_analyzer):
        result = detect_pii(text)
...
def test_whitelist_filters_scrub_pii(mock_analyzer_global):
    ...
    text = "Problem is the next release by TestUser"
    mock_analyzer = MagicMock()
    mock_analyzer.analyze.return_value = [
        _mock_analyzer_result("PERSON", 0, 7, 0.70),       # "Problem"
        _mock_analyzer_result("PERSON", 31, 36, 0.85),     # "TestUser"
    ]
    mock_anonymizer = MagicMock()
    with patch("core.safety.pii.get_analyzer", return_value=mock_analyzer), \
         patch("core.safety.pii.get_anonymizer", return_value=mock_anonymizer):
...
def test_whitelist_case_insensitive(mock_analyzer_global):
    ...
    text = "RULEZ and FRONTPOINT"
    mock_analyzer = MagicMock()
    mock_analyzer.analyze.return_value = [
        _mock_analyzer_result("PERSON", 0, 5, 0.85),         # "RULEZ"
        _mock_analyzer_result("ORGANIZATION", 10, 20, 0.80),  # "FRONTPOINT"
    ]
    with patch("core.safety.pii.get_analyzer", return_value=mock_analyzer):
        result = detect_pii(text)
...
def test_whitelist_mixed_keeps_real_pii(mock_analyzer_global):
    ...
    mock_analyzer = MagicMock()
    mock_analyzer.analyze.return_value = [
        _mock_analyzer_result("PERSON", 6, 11, 0.85),         # "TestUser" (whitelisted)
