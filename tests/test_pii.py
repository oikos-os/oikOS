...
def test_whitelist_filters_detect_pii(mock_analyzer_global):
    """Whitelisted terms (owner personas + framework names) are not flagged as PII."""
    text = "Ask TestUser about Apple Card"
    mock_analyzer = MagicMock()
    mock_analyzer.analyze.return_value = [
        _mock_analyzer_result("PERSON", 4, 12, 0.85),        # "TestUser" (fixture nickname)
        _mock_analyzer_result("ORGANIZATION", 19, 29, 0.80),  # "Apple Card" (framework)
    ]
    with patch("core.safety.pii.get_analyzer", return_value=mock_analyzer):
        result = detect_pii(text)
...
def test_whitelist_filters_scrub_pii(mock_analyzer_global):
    ...
    text = "Ask Alice about TestUser"
    mock_analyzer = MagicMock()
    mock_analyzer.analyze.return_value = [
        _mock_analyzer_result("PERSON", 4, 9, 0.70),        # "Alice" (fixture first_name)
        _mock_analyzer_result("PERSON", 16, 24, 0.85),      # "TestUser" (fixture nickname)
    ]
    mock_anonymizer = MagicMock()
    with patch("core.safety.pii.get_analyzer", return_value=mock_analyzer), \
         patch("core.safety.pii.get_anonymizer", return_value=mock_anonymizer):
...
def test_whitelist_case_insensitive(mock_analyzer_global):
    ...
    text = "TESTUSER and APPLE CARD"
    mock_analyzer = MagicMock()
    mock_analyzer.analyze.return_value = [
        _mock_analyzer_result("PERSON", 0, 8, 0.85),          # "TESTUSER"
        _mock_analyzer_result("ORGANIZATION", 13, 23, 0.80),  # "APPLE CARD"
    ]
    with patch("core.safety.pii.get_analyzer", return_value=mock_analyzer):
        result = detect_pii(text)
...
def test_whitelist_mixed_keeps_real_pii(mock_analyzer_global):
    ...
    text = "Email TestUser and John at john@example.com"
    mock_analyzer = MagicMock()
    mock_analyzer.analyze.return_value = [
        _mock_analyzer_result("PERSON", 6, 14, 0.85),         # "TestUser" (whitelisted)
        _mock_analyzer_result("PERSON", 19, 23, 0.90),        # "John" (NOT whitelisted)
        _mock_analyzer_result("EMAIL_ADDRESS", 27, 43, 0.95), # john@example.com
    ]
    with patch("core.safety.pii.get_analyzer", return_value=mock_analyzer):
        result = detect_pii(text)
    ...
    # The PERSON entity should be "John", not "TestUser"
