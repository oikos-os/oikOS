...
def _setup_projects(tmp_path):
    ...
    projects.write_text(
        "# PROJECTS\n"
        "## 1. PROJECT TRENDY DECAY (THE LABEL)\n"
        "## 2. PROJECT OIKOS (THE BRAIN)\n"
        "## 3. PROJECT ARCADIA HEIGHTS (THE FRANCHISE)\n"
...
def _setup_oikos_logs(tmp_path, days=3, music_days_ago=None):
    ...
    If music_days_ago is set, also create a trendy decay session that many days ago.
    """
    ...
    if music_days_ago is not None:
        ...
        log_file.write_text("# Session\nWorked on trendy decay production.", encoding="utf-8")
...
def test_parse_projects(tmp_path):
    ...
    assert len(result) == 3
    assert result[0]["name"] == "TRENDY DECAY"
...
def test_parse_projects_generates_keywords(tmp_path):
    projects = tmp_path / "PROJECTS.md"
    projects.write_text(
        "## 1. PROJECT ARCADIA HEIGHTS (THE FRANCHISE)\n",
        encoding="utf-8",
    )
    result = parse_projects(projects)
...
def test_build_domain_keyword_map(tmp_path):
    ...
    domain_map = build_domain_keyword_map(tmp_path / "identity" / "PROJECTS.md")
    assert "trendy decay" in domain_map
    assert "trendy" in domain_map["trendy decay"]
    assert "oikos" in domain_map


def test_build_domain_keyword_map_missing_file():
    domain_map = build_domain_keyword_map(Path("/nonexistent/PROJECTS.md"))
...
def test_infer_domain_by_track_keyword():
    domain_map = {"trendy decay": {"trendy", "decay", "horas"}}
    dl = {"project": "Vossa", "track": "Horas"}
    assert _infer_domain_for_deadline(dl, domain_map) == "trendy decay"


def test_infer_domain_fallback_music():
    domain_map = {"trendy decay": {"trendy", "decay"}, "oikos": {"oikos"}}
...
def test_get_session_activity_uses_domain_map(tmp_path):
    ...
    log_file.write_text("# Session\nWorked on trendy decay stuff.", encoding="utf-8")
    ...
    assert "trendy decay" in activity[0]["domain_hints"]
...
def test_days_since_domain_activity_found():
    activity = [
        ...
        {"date": _now() - timedelta(days=5), "source": "test", "domain_hints": {"trendy decay"}},
    ]
    days = _days_since_domain_activity(activity, "trendy decay")
...
def test_generate_nudges_no_drift_when_active(tmp_path):
    ...
    log_file.write_text("# Session\nWorked on trendy decay music production.", encoding="utf-8")
...
def test_advisory_with_pattern_match(tmp_path):
    ...
    # Create LEARNED.md with failure pattern for trendy decay
    identity_dir = tmp_path / "identity"
    learned = identity_dir / "LEARNED.md"
    learned.write_text("- Trendy Decay releases often stall due to mastering delays.\n", encoding="utf-8")
...
def test_dismissal_promotes_nudge_to_advisory(tmp_path):
    ...
    pid = _pattern_id("trendy decay", "Secrets")
    state = {"patterns": {pid: {
        "times_surfaced": 2,
        "times_dismissed": 2,
...
def test_find_failure_pattern_match(tmp_path):
    ...
    identity_dir.mkdir(parents=True, exist_ok=True)
    learned = identity_dir / "LEARNED.md"
    learned.write_text("- Trendy decay releases often stall when mastering.\n", encoding="utf-8")

    result = _find_failure_pattern("trendy decay", vault_dir=tmp_path)
