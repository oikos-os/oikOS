...
def test_clean_normal_response():
    ...
    response = (
        ...
        "Modules 2-5 await ARCHITECT's green light."
    )
...
def test_critical_anthropic_sk_ant_api03_suppressed():
    """Anthropic sk-ant-api03 format key is hard-suppressed (hyphenated body)."""
    response = (
        "Your Anthropic API key is sk-ant-api03-abc123def456ghi789jkl012mno345 — "
        "use it to call Claude."
    )
    result = check_output_sensitivity(response)
    ...
    assert result.action == "suppressed"
    assert result.response == _SUPPRESS_MESSAGE
    assert "sk-ant-api03" not in result.response


def test_false_positive_sk_antenna_not_flagged():
    """Short strings like 'sk-antenna' must not trigger the api_key pattern."""
    response = "The sk-antenna was configured for long-range reception."
    result = check_output_sensitivity(response)
    assert "api_key_prefixed" not in result.triggered


def test_false_positive_sk_antibody_not_flagged():
    """Short strings like 'sk-antibody' must not trigger the api_key pattern."""
    response = "The sk-antibody test returned negative results."
    result = check_output_sensitivity(response)
    assert "api_key_prefixed" not in result.triggered
...
def test_handler_integration_step_8c_fires(tmp_path):
    ...
    with (
        ...
        patch("core.cognition.pipeline.classify.detect_pii", return_value=MagicMock(has_pii=False, entities=[])),
        patch("core.cognition.pipeline.context.compile_context", return_value=MagicMock(slices=[], total_tokens=0, budget=6000, query="q")),
        patch("core.cognition.pipeline.dispatch.render_context", return_value="ctx"),
        patch("core.cognition.pipeline.context.load_system_prompt", return_value="sys"),
        patch("core.cognition.routing.route_query", return_value=MagicMock(
        ...
        patch("core.cognition.pipeline.dispatch.generate_local", return_value={"response": sensitive_response, "logprobs": None}),
        patch("core.cognition.pipeline.postprocess.score_response", return_value=MagicMock(score=75.0, method="test", hedging_flags=[])),
