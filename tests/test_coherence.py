...
# Neutral filler: no KAIROS markers, no foreign persona patterns
...
# 35-word response with two KAIROS identity markers ("vault", "phase")
_DENSE_35 = (
...
# 30-word pirate response — foreign markers (nautical_slang), zero KAIROS markers
_PIRATE_30 = (
...
# 30-word pirate response WITH one KAIROS marker ("phase") → density = 3.33/100 >> 0.5
_PIRATE_VAULT_30 = (
    "Ahoy matey! I am the captain. This vault of our voyage is critical. "
...
def test_single_marker_at_30_words_pass():
    """30 neutral words + 'vault' inserted → density >> 0.3 → pass."""
    response = _NEUTRAL_29 + " vault"  # 30 words, 1 marker
...
def test_critical_foreign_zero_identity_hard_veto():
    """Pirate roleplay (30 words), zero KAIROS identity → HARD VETO, is_coherent=False."""
...
def test_high_foreign_with_identity_soft_veto():
    """Pirate roleplay (30 words) + 'vault' marker → density >> 0.5 → SOFT VETO."""
    result = check_coherence(_PIRATE_VAULT_30)
    assert result.is_coherent is False
    assert result.foreign_persona_detected is True
    assert result.warning_message is not None
