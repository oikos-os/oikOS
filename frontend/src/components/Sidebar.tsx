  ...
  return (
    ...
    >
      ...
      <div className="flex items-center h-12 px-3 gap-2">
        ...
        <button
          onClick={onNewChat}
          className={`text-sm font-bold tracking-widest text-white whitespace-nowrap overflow-hidden transition-opacity duration-200 hover:text-amber-400 phosphor-glow phosphor-alive ${collapsed ? "opacity-0 w-0" : "opacity-100"}`}
          title="New Chat"
        >
          oikOS
        </button>
      ...
      <button
        onClick={onNewChat}
        className="flex items-center gap-2 mx-2 mb-2 px-2 py-1.5 text-sm text-neutral-300 hover:bg-[var(--bg-elevated)] hover:text-white transition-colors"
      ...
      <button
        ...
        className={`flex items-center gap-2 px-4 py-2.5 text-sm border-t border-[var(--border-subtle)] shrink-0 transition-colors ${
          page === "settings" ? "bg-[var(--bg-elevated)] text-white" : "text-neutral-500 hover:text-neutral-300"
        }`}
      >
...
function ChatEntry({ chat, starred, onOpen, onToggleStar }: {
  ...
  return (
    <div className="group flex items-center gap-1 px-2 py-1 hover:bg-[var(--bg-elevated)] transition-colors">
      <button
      ...
      <span className="text-[10px] text-neutral-600 shrink-0">{timeAgo(chat.started_at, true)}</span>
      <button
