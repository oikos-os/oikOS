...
export default function MessageInput({ onSend, disabled, roomModel }: Props) {
  const ref = useRef<HTMLTextAreaElement>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [modelOpen, setModelOpen] = useState(false);
  const [models, setModels] = useState<ModelInfo[]>([]);
  const [selectedModel, setSelectedModel] = useState<string>(roomModel || "qwen2.5:14b");
  const [attachedFiles, setAttachedFiles] = useState<{ name: string; content: string }[]>([]);
  const selectedModelRef = useRef(selectedModel);
  selectedModelRef.current = selectedModel;

  // Sync with room model default when room changes
  useEffect(() => {
    if (roomModel) setSelectedModel(roomModel);
  }, [roomModel]);
