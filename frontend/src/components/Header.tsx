import type { SystemState, RoomConfig } from "../types";
...
export default function Header({ state, wsConnected, activeModel, onThemeToggle, theme, activeRoom, onRoomSwitch, onRoomUpdated }: Props) {
  ...
  return (
    <header className="flex items-center justify-end px-4 py-1.5 border-b border-[var(--border-subtle)] text-sm">
      <div className="flex items-center gap-5">
        {activeRoom && onRoomSwitch && (
          <RoomSwitcher activeRoom={activeRoom} onSwitch={onRoomSwitch} />
        )}
        {activeRoom && (
          <ModelSelector activeRoom={activeRoom} onRoomUpdated={onRoomUpdated} lastUsedModel={activeModel} />
        )}
