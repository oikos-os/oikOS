...
export default function StepInference({
  ...
  const [oauthDiscovery, setOauthDiscovery] = useState<{ found: boolean; subscription_type?: string } | null>(null);

  useEffect(() => {
    fetch("/api/auth/claude/discover")
      .then(r => r.ok ? r.json() : null)
      .then(setOauthDiscovery)
      .catch(() => {});
  }, []);
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          {oauthDiscovery?.found && (
            <button
              data-testid="oauth-detection-card"
              onClick={async () => {
                try {
                  ...
                  setError("Failed to connect — try using an API key instead.");
                } catch {
                  setError("Connection error — try using an API key instead.");
                }
              }}
              className="w-full text-left bg-[var(--bg-elevated)] border-2 border-[var(--accent)] p-4 transition-colors hover:bg-[var(--bg-tertiary)]"
            >
              <span className="text-[var(--accent)] font-medium block">
                {"\u25c8"} Claude Code detected
              </span>
              <span className="text-[var(--text-secondary)] text-sm">
                Your {oauthDiscovery.subscription_type} subscription is available. No API key needed.
              </span>
              <span className="text-[var(--text-muted)] text-xs block mt-1">
                Click to connect with Claude Code
              </span>
            </button>
          )}
          ...
          <button
            ...
            className="w-full text-left bg-[var(--bg-secondary)] border border-neutral-800 hover:border-amber-400/50 p-4 transition-colors"
          >
      ...
      {showLocal && (
        <div className="space-y-4">
          ...
          {scanning ? (
            ...
            <div className="bg-[var(--bg-secondary)] border border-neutral-800 p-6 text-center space-y-3">
