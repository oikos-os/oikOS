...
export default function RoomSwitcher({ activeRoom, onSwitch }: Props) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <span className="w-1.5 h-1.5 bg-amber-400 rounded-full phosphor-alive" />
      ...
      {open && rooms.length > 0 && (
        <div className="absolute right-0 top-full mt-1 w-48 bg-[var(--bg-secondary)] border border-neutral-800 shadow-lg z-50">
          {rooms.map(r => (
            <button
              key={r.id}
              onClick={() => {
                const screenEl = document.querySelector('.oikos-screen');
                if (screenEl) {
                  const effectsEnabled = getComputedStyle(document.documentElement)
                    .getPropertyValue('--effects-enabled').trim();
                  if (effectsEnabled === '1') {
                    screenEl.classList.add('room-switching');
                    setTimeout(() => screenEl.classList.remove('room-switching'), 200);
                  }
                }
                onSwitch(r.id);
                setOpen(false);
              ...
              className={`w-full text-left px-3 py-1.5 text-xs transition-colors ${
                r.id === activeRoom.id
                  ? "text-amber-400 bg-[var(--bg-elevated)]"
                  : "text-neutral-400 hover:text-white hover:bg-[var(--bg-tertiary)]"
              }`}
            >
              {r.name}
