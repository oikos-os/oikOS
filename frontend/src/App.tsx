...
import type { RoomConfig, OnboardingStatus } from "./types";
...
function AppMain() {
  ...
  // Apply CRT effects preference from localStorage on mount
  useEffect(() => {
    try {
      const stored = localStorage.getItem("oikos-crt-effects");
      const enabled = stored !== "0";
      document.documentElement.style.setProperty("--effects-enabled", enabled ? "1" : "0");
      document.documentElement.style.setProperty("--scanline-opacity", enabled ? "0.08" : "0");
      document.documentElement.style.setProperty("--flicker-play", enabled ? "running" : "paused");
    } catch { /* localStorage unavailable */ }

    // Load phosphor theme from settings API
    fetch("/api/settings")
      .then(r => r.ok ? r.json() : {})
      .then((settings: Record<string, unknown>) => {
        const phosphorTheme = (settings.theme as string) || "amber";
        document.documentElement.classList.remove("theme-green", "theme-white");
        if (phosphorTheme !== "amber") {
          document.documentElement.classList.add(`theme-${phosphorTheme}`);
        }
      })
      .catch(() => {});
  }, []);
  ...
  const handleRoomSwitch = useCallback(async (roomId: string) => {
    const resp = await fetch("/api/rooms/switch", {
      method: "POST",
  ...
  const { toasts, dismiss, addToasts } = useNotifications(events);
  usePendingNotifications(addToasts);
  ...
  return (
    <div className="oikos-screen h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] flex overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-screen min-w-0 overflow-hidden">
        <Header state={state} wsConnected={connected} activeModel={chatState.activeModel} onThemeToggle={toggleTheme} theme={theme} activeRoom={activeRoom} onRoomSwitch={handleRoomSwitch} onRoomUpdated={setActiveRoom} />
        ...
        {page === "chat" && chatState.messages.length > 0 && (
          <ChatView chatState={chatState} roomModel={activeRoom?.model?.model} />
        )}
...
export default function App() {
  ...
  useEffect(() => {
    fetch("/api/onboarding/status")
      .then(r => r.ok ? r.json() : { complete: true })
      .then((s: OnboardingStatus) => setOnboardingComplete(s.complete))
