...
export function useNotifications(events: EventRecord[] | null, prefs?: Partial<NotificationPrefs>) {
  ...
  // Exposed so external hooks (e.g. usePendingNotifications) can inject toasts
  // without duplicating auto-dismiss state.
  const addToasts = useCallback((incoming: Toast[]) => {
    setToasts((prev) => [...prev, ...incoming]);
  }, []);

  return { toasts, dismiss, addToasts };
}
