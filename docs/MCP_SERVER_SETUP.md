...
| Permission denied on file ops | FileAgent scope is loaded from `config/owner_identity.toml` under `[paths.file_agent]`. Populate it with READ / READ_WRITE entries for the directories you want to expose. |
