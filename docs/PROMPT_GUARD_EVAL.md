...
- OIKOS is a **sovereign, local-first system** with a single operator (ARCHITECT). The threat model is self-injection via RAG (vault content manipulating inference), not untrusted external users.
